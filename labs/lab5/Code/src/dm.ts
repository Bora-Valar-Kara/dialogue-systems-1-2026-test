...
const azureLanguageCredentials = {
  endpoint: "https://switzerlandnorthlanguageresource.cognitiveservices.azure.com/language/:analyze-conversations?api-version=2024-11-15-preview" /** your Azure CLU prediction URL */,
  key: NLU_KEY /** reference to your Azure CLU key */,
  deploymentName: "NLUModelForDM4" /** your Azure CLU deployment */,
  projectName: "TA-ExampleProject" /** your Azure CLU project name */,
};
...
const grammar: { [index: string]: GrammarEntry } = { // Grammar will not be important as it will be replaced by intents and entities in the Azure CLU, but we will keep it for the sake of the example. You can use it to test the DM without using the NLU.
  //some dummy people
...
// 1. We no longer have isInGrammar-like functions, as we will rely on the NLU to extract the intents and entities from the user's utterances.
// 2. In the grammar version, we had the getPerson functions like this
...
// But now that won't work, because we will not rely on the grammar to extract the person from the user's utterance, but we will rely on the NLU to extract the person from the user's utterance. So we have to write a new function to extract the person from the interpretation in the context.
...
function getPerson(context: DMContext): string | null {
  if (context.interpretation) { // If the interpretation exists in the context
    for (const entity of context.interpretation.entities) { // Loop through the entities in the interpretation
...
const dmMachine = setup({
  ...
  actions: {
    ...
    "spst.listen": ({ context }) =>
      context.spstRef.send({
        type: "LISTEN",
        value: { nlu: true } // Attention here. This line is important. It tells the SpeechState machine to use the NLU.
      }),
  ...
  context: ({ spawn }) => ({
    ...
    Hour: null,
    confirm: null,
    interpretation: null, // Attention here.
  }),
    ...
    Greeting: { 
      ...
      on: { 
        LISTEN_COMPLETE: [
          {
            target: "WhoIsPerson", 
            guard: ({ context }) => context.interpretation?.topIntent === "WhoIsPerson", // Attention here
            actions: assign(({ context }) => ({ Person: getPerson(context) })),
          },

          { target: "MeetWithPerson", 
            guard: ({ context }) => context.interpretation?.topIntent === "MeetWithPerson", // Attenton here
          ...
          on: {
            RECOGNISED: { //RECOGNISED is an event that is sent by the SpeechState machine when it recognizes the speech
              actions: assign(({ event }) => {
                return { lastResult: event.value, interpretation: event.nluValue }; // We will store the interpretation in the context
              }),
            },
    ...
    AskForAnyOtherQuestion: { 
      ...
      on: { 
        LISTEN_COMPLETE: [
          ...
          {
            target: "WhoIsPerson", 
            guard: ({ context }) => context.interpretation?.topIntent === "WhoIsPerson", 
            actions: assign(({ context }) => ({ Person: getPerson(context) })),
          },

          { target: "MeetWithPerson", 
            guard: ({ context }) => context.interpretation?.topIntent === "MeetWithPerson",
            actions: assign(({ context }) => ({ Person: getPerson(context), Day: getDay(context), Hour: getHour(context) })),
          },
    ...
    MeetWithPerson: {
      entry:{
        ...
        params: ({ context }) => {
          ...
          if (person && day && hour) { // if all the information is present in the context, we can arrange the meeting
